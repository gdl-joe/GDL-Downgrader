...
// Entfernt Converter, die über den kanonischen Pfad auf dieselbe Datei zeigen.
// Nötig, weil unter Windows die Basisverzeichnisse GRAPHISOFT und Graphisoft auf
// denselben (case-insensitiven) Ordner verweisen und sonst jeden Converter doppeln.
function dedupeByPath(list, platform) {
  const seen = new Set();
  const out = [];
  for (const c of list) {
    let key;
    try { key = fs.realpathSync.native(c.path); } catch (e) { key = c.path; }
...
function scanConverters(platform = process.platform, baseDirs = null) {
  ...
  return dedupeByPath(found, platform).sort((a, b) => (b.version || 0) - (a.version || 0));
}
